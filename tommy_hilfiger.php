...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: 'tommy_hilfiger_getData.php',
    ...
    //get number of rows
    $queryNum = $db->query("SELECT COUNT(*) as postNum FROM tommyhilfiger");
    ...
    //get rows
    $query = $db->query("SELECT * FROM tommyhilfiger ORDER BY id DESC LIMIT $limit");
   $output ="";
$i = 0;
 if($query->num_rows > 0){
